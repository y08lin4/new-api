...
func GetTopUpInfo(c *gin.Context) {
	...
	affiliatePolicy, err := model.GetAffiliatePolicyForUser(c.GetInt("id"))
	if err != nil {
...
func EpayNotify(c *gin.Context) {
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		if err := model.RechargeEpay(verifyInfo.ServiceTradeNo, verifyInfo.Type, c.ClientIP()); err != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("epay topup settlement failed trade_no=%s callback_type=%s client_ip=%s error=%q", verifyInfo.ServiceTradeNo, verifyInfo.Type, c.ClientIP(), err.Error()))
			return
		}
	} else {
